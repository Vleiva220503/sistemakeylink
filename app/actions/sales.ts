...
export async function createSale(payload: CreateSalePayload) {
  ...
  try {
    ...
    const { data: saleId, error } = await (supabase as any).rpc('create_sale', {
      ...
      p_delivery_type: payload.delivery_type || null
    })
...
export async function voidSale(saleId: string) {
  ...
  try {
    ...
    if (!payErr && payments) {
      ...
      // If delivery_type='external', that delivery amount was collected from the customer
      // but paid directly to the third-party courier — it NEVER entered the cash register.
      // The original create_sale already excluded it from the cash_movement when the sale
      // was created, so the reversal must exclude it too.
      // For delivery_type='own' (or null/no delivery), deliveryExterno=0, so the full
      // cashPaid is reversed — which is the correct behavior.
      const deliveryExterno =
        (sale as any).delivery_type === 'external'
          ? Number((sale as any).delivery_amount || 0)
          : 0

      // Amount that actually entered the register for this sale
      const montoRealEnCaja = cashPaid - deliveryExterno

      if (montoRealEnCaja > 0) {
        const { error: cashMoveErr } = await (supabase as any)
          .from('cash_movements')
          .insert({
            register_id: (sale as any).register_id,
            type: 'adjustment',
            amount: -montoRealEnCaja, // Reverses exactly what entered the register
