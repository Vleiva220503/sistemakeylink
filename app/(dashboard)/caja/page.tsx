...
export default async function CajaPage({ searchParams }: PageProps) {
  ...
  // Build movements query with optional date filter
  ...
  if (startDate) {
    movQuery = movQuery.gte('created_at', `${startDate}T00:00:00-06:00`)
  }
  if (endDate) {
    movQuery = movQuery.lte('created_at', `${endDate}T23:59:59-06:00`)
  }

  const [{ data: regRows }, { data: movRows }] = await Promise.all([
    (supabase as any).from('cash_registers').select('*').order('name'),
    movQuery.order('created_at', { ascending: false }).limit(500),
  ])

  const registers = (regRows as any[]) || []
  const movements = (movRows as any[]) || []

  // Fetch summaries for all open registers (always reflects the full current shift — no date filter)
  ...
  // Bloque 2 — Global Summary: recalculated from the (possibly date-filtered) movements
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Active Shift Summaries — always reflects the full current shift, no date filter */}
              ...
              <Card key={idx} className="border-2 border-primary/20 bg-primary/5">
                ...
                <CardContent className="space-y-4">
                  ...
                  {Number(summary?.totalExternalDelivery || 0) > 0 && (
                    <div className="pt-2 border-t border-border/60 flex items-center justify-between text-xs bg-amber-500/10 p-2 rounded-md border border-amber-500/20 text-amber-700 dark:text-amber-400">
      ...
      {/* Global Summary — recalculated from the date-filtered movements */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Ingresos (ventas en efectivo){startDate || endDate ? ' — rango filtrado' : ''}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="pt-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Egresos{startDate || endDate ? ' — rango filtrado' : ''}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="pt-5">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Balance Neto{startDate || endDate ? ' — rango filtrado' : ''}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      {/* Movements — client component with date filter UI */}
      <MovementsClient
        movements={movements}
        startDate={startDate || ''}
        endDate={endDate || ''}
